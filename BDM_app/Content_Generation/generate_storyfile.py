...
if __name__ == '__main__':
    ...
    parser.add_argument("--file", help="txt file from which to generate content (must follow conventions)")
    args = parser.parse_args()
    name = args.file.split('.txt')[0]

    cwd = os.getcwd()
    # extract_nodes(cwd+'/Prologue_Conclave_des_elfes_v1.docx')
    parse_file(cwd+'/'+name+'.txt', cwd+'/'+name+'.json')
